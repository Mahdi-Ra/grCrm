...
<div class="col-md-12  col-lg-7 p-b-9 align-self-center {{ app()->getLocale() == 'persian' ? 'text-left' : 'text-right' }} {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        @if(config('visibility.action_buttons_change_owner'))
        <button type="button" title="{{ cleanLang(__('lang.change_account_owner')) }}" id="clients-import-button"
            ...
            data-action-url="{{ urlResource('/clients/'.request('contactresource_id').'/change-account-owner?ref=page') }}" data-action-method="POST"
            data-loading-target="commonModalBody" data-action-ajax-loading-target="commonModalBody"
            data-action-ajax-class="ajax-request"
            data-modal-title="@lang('lang.change_account_owner')" data-url="{{ urlResource('/clients/'.request('contactresource_id').'/change-account-owner?ref=page') }}">
