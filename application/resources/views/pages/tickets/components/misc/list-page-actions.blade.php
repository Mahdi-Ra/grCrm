...
<div class="col-md-12  col-lg-7 p-b-9 align-self-center {{ app()->getLocale() == 'persian' ? 'text-left' : 'text-right' }} {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        <!--ARCHIVED TICKETS-->
        @if(config('visibility.archived_tickets_toggle_button'))
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.show_archive_tickets')) }}"
            id="pref_filter_show_archived_tickets"
            class="list-actions-button btn btn-page-actions waves-effect waves-dark js-ajax-ux-request {{ runtimeActive(auth()->user()->pref_filter_show_archived_tickets) }}"
            data-url="{{ urlResource('/tickets/search?action=search&toggle=pref_filter_show_archived_tickets') }}">
